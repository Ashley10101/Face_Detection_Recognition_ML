...
class PCA_Processor(object):
    def __init__(self,label_name):
        ...
        self.name_index = os.listdir("face")
    ...
    def test_recognize(self,test_img,img_name):
        ...
        orig_img = cv2.imread("face"+'/'+str(img_name))
        ...
        face_aligment = FACE_ALIGMENT()
        # Get the rotation angle from the original image
        ...
        data_row = self.data_prerocess(gray)
        data_row = np.array(data_row)
        test_dataset = double(data_row) / 255 # Light compensation

        test_data = float32(mat(test_dataset)) # Transforming data into a matrix
        test_number,testsize = test_data.shape # Take the size of the test set
        trainNumber,testsize = Matrix.shape

        # Calculate the difference between each vector and the average vector
        data = float32(mat(Matrix))
        data_mean = mean(data,0)

        temp_face = test_data - data_mean
        data_test_new = temp_face*self.Vector # Get the test face under the feature vector
        data_test_new = np.array(data_test_new) # mat change to array
        data_train_new = np.array(data_new)
        ...
        sqDiffMat = diffMat**2
        sqDistances = sqDiffMat.sum(axis=1)
        sortedDistIndicies = sqDistances.argsort() # Find the index of the shortest Euclidean distance
        result_label = self.Train_Labels[sortedDistIndicies[0]]
